...
const DesignConfigurator = ({configId, imageUrl, imageDimensions}:DesignConfiguratorProps) => {
    ...
    function base64ToBlob(base64Data:string,mimeType:string){
        ...
        for (let i = 0; i < byteCharacters.length; i++){
            byteNumbers[i] = byteCharacters[i].charCodeAt(i)
        }
    ...
    <div className='relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20'>
        <div ref={containerRef} className='relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2'>
            <div className='relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831] '>
                ...
                <div className='absolute z-40 inset-0 left-[3px] top-px right-[3px] bottom-px rounded-[32px] shadow-[0_0_0_99999px_rgba(22,231,235,0.6)]'/>
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent>
                                    {MODELS.options.map((model) => (
                                        ...
                                        >
                                            <Check className={cn("mr-2 size-4", model.label === options.model.label
                                                ? 'opacity-100'
                                                : 'opacity-0'
                                            )}/>
                                            {model.label}
                                ...
                                <div className='mt-3 space-y-4'>
                                    {selectableOptions.map((option) => (
                                        <RadioGroup.Option 
                                            key={option.value}
                                            value = {option}
                                            className={({active, checked}) => cn("relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 sm:flex sm:justify-between", {
